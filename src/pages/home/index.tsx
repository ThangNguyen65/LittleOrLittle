...
function Home() {
  ...
  const today = dayjs();
  // Hàm kiểm tra xem một ngày có thuộc tháng hiện tại hay không
  const isInCurrentMonth = (date: any) => {
    const currentMonth = dayjs().format("YYYY-MM");
    return dayjs(date).format("YYYY-MM") === currentMonth;
  };
  const hideNonCurrentMonthDates = (current: any) => {
    return !isInCurrentMonth(current);
  };
  // Kiểm tra xem ngày hiện tại có phải là Chủ nhật (0) và không phải ngày hôm nay
  if (today.day() === 0 && !today.isSame(today, "day")) {
    // Nếu là Chủ nhật, cộng thêm 1 ngày để chuyển sang thứ Hai
    const nextMonday = today.add(1, "day").format("DD/MM/YYYY");
    setDateUsed(nextMonday);
  }
                ...
                >
                  ...
                  <div className="button-container-date">
                    ...
                    <DatePicker
                      picker="date"
                      allowClear={false}
                      className="btn_date custom-date-picker"
